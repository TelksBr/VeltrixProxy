...
func TestXrayOptionFormatting(t *testing.T) {
	languages := []i18n.Language{i18n.LangPT, i18n.LangES, i18n.LangEN}
	width := components.DefaultBoxWidth
	...
	for _, lang := range languages {
		...
		line := advMenuItem("X", i18n.T("adv_opt_xray"), components.FormatBool(true))

		visible := theme.VisibleLen(line)
		contentWidth := width - 4
		if visible > contentWidth {
			t.Errorf("Idioma %s: opção Xray visível (%d) excede contentWidth (%d): %q", lang, visible, contentWidth, line)
		}
